...
import platform		# Load Modules, this is multi platform, or different architecture adaption
import os           
import subprocess
import sys

from time import strftime		# Load just the strftime Module from Time, various time functions like calender.

def clear_screen():				# Function to clear the screen
    if os.name == "posix":		# Unix/Linux/MacOS/BSD/etc, dang this is old people stuff
...
def print_docs():							# Function to print the daily checks automatically
  ...
  subprocess.Popen(["C:\\Program Files (x86)\Microsoft Office\Office14\winword.exe", "P:\\\\Documentation\\Daily Docs\\Back office Daily Checks.doc", "/mFilePrintDefault", "/mFileExit"]).communicate() #allows us to make more than process, we are using communicate as well, so 
                                                                #the process is PIPE
  
def putty_sessions():						# Function to load the putty sessions I need, PUTTY?!?!?!?!
                                            #it turns out PuTTY is a terminal session emulator
  ...
  for server in open(conffilename):			# Open the file server_list.txt, loop through reading each line - 1.1 -Changed - 1.3 Changed name to use variable conffilename
    subprocess.Popen(('putty -load '+server))	# Open the PuTTY sessions - 1.1 we open a new process thats a server

def rdp_sessions():
  print "Loading RDP Sessions:"   #remote desktop protocol session
  subprocess.Popen("mstsc eclr.rdp")		# Open up a terminal session connection and load the euroclear session
  
def euroclear_docs():
  # The command below opens IE and loads the Euroclear password document, whats the euroclear document???
...
def main():
    filename = sys.argv[0]							# Create the variable filename, argv[0] isnt that executable name??????
    ...
    print "Good Morning " + os.getenv('USERNAME') + ", " + filename, "ran at", strftime("%Y-%m-%d %H:%M:%S"), "on",platform.node(), "run from",os.getcwd()  #getting current working directory
...
if __name__ == '__main__':          #again, this module is only written for being non imported like the last example we looked at.
    main()
